...
async def test_pipeline_generates_successful_playlist_for_geolocated_user(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=digital_items, calls=[RetrievalPredictionResult(status="success")]
    )
...
async def test_pipeline_generates_successful_playlist_without_gps(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=[*digital_items, geolocated_item], calls=[RetrievalPredictionResult(status="success")]
    )
    mock_vertex_ranking[0].side_effect = lambda offers, _ctx: RankingPredictionResult(status="success", offers=offers)
...
async def test_pipeline_filters_out_already_booked_items(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=[item_a, item_b, item_c], calls=[RetrievalPredictionResult(status="success")]
    )
...
async def test_pipeline_handles_venues_out_of_range(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=[item_close, item_far, item_multi], calls=[RetrievalPredictionResult(status="success")]
    )
    mock_vertex_ranking[0].side_effect = lambda offers, _ctx: RankingPredictionResult(status="success", offers=offers)
    mocker.patch("controllers.pipeline_playlist_recommendation.log_past_offer_context_to_sink")
...
async def test_pipeline_applies_diversification_correctly(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=[], calls=[RetrievalPredictionResult(status="success")]
    )
    mocker.patch(
        "controllers.pipeline_playlist_recommendation.resolve_closest_venues_from_items",
        new_callable=mocker.AsyncMock,
...
async def test_pipeline_handles_new_user_cold_start(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=cold_start_items, calls=[RetrievalPredictionResult(status="success")]
    )
...
async def test_pipeline_skips_tracking_for_unauthenticated_user(
    ...
    mock_vertex_retrieval[0].return_value = AggregatedRetrievalResult(
        predictions=unauthenticated_items, calls=[RetrievalPredictionResult(status="success")]
    )
    mock_vertex_ranking[0].side_effect = lambda offers, _ctx: RankingPredictionResult(status="success", offers=offers)
    mock_tracking = mocker.patch("controllers.pipeline_playlist_recommendation.log_past_offer_context_to_sink")
