...
async def __get_model(endpoint_name, location):
    # aiplatform.Endpoint.list() is synchronous — run in a thread to avoid blocking the event loop.
...
async def get_client(api_endpoint):
    ...
    # PredictionServiceAsyncClient: await client.predict() releases the event loop
    # during the network call, so asyncio.gather() runs predictions truly in parallel.
    return aiplatform.gapic.PredictionServiceAsyncClient(client_options=client_options)
