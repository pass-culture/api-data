...
from huggy.core.model_engine.factory import (
    ModelEngineFactory,
    ModelEngineOut,
...
@r.post(
    "/playlist_recommendation_with_scores/{user_id}",
    dependencies=[Depends(setup_trace), Depends(check_token)],
)
async def playlist_recommendation_with_scores(
    user_id: str,
    playlist_params: p.PlaylistParams,
    token: str,
    latitude: Optional[float] = None,
    ...
    model_engine_out_scores: ModelEngineOutScores = (
        await ModelEngineFactory.handle_prediction_with_scores(
            db,
            user=user,
            params_in=playlist_params,
            call_id=call_id,
    ...
    return jsonable_encoder(
        {
            "playlist_recommended_offers": model_engine_out_scores.results,
            "offers_scores": model_engine_out_scores.scores,
            "params": {
                "reco_origin": model_engine_out_scores.model.reco_origin,
                "model_origin": model_engine_out_scores.model.model_origin,
                "call_id": call_id,
            },
        }
    )
