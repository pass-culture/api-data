...
class PreprocessTest:
    ...
    def test_prepare_features(self, input_body, expected_preprocess):
        assert (
            prepare_features(input_body, api_config.preprocess_features_type)
            == expected_preprocess
        )
    ...
    def test_extract_embedding(
        ...
        expected_preprocess,
        expected_data_embedding,
    ):
        prepoc_models = {}
        for (
            feature_type
        ) in model_config_default.pre_trained_model_for_embedding_extraction.keys():
            prepoc_models[feature_type] = model_handler_default.get_model_by_name(
                feature_type
            )
        encode_from_feature_mock.return_value = "text embedding"
        encode_from_url_mock.return_value = "image embedding"
        output = extract_embedding(
            expected_preprocess, api_config.features_to_extract_embedding, prepoc_models
        )
